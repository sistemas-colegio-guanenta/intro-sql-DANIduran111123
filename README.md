...
`SELECT id_fabricante, AVG(precio_articulo) AS  precio_articulo FROM articulo  GROUP BY id_fabricante;`
...
`SELECT f.nombre_fabricante, AVG (a.precio_articulo)AS precio_promedio FROM articulo  a JOIN fabricantef ON a.id_fabricante = f.id_fabricante GROUP BY F.nombre_fabricante;`
